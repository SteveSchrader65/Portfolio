...
/*
// Function to check if modifications have been made to inputs
function checkForm () {
    'use strict';
    ...
    localValues.forEach(function(storedAttribute) {
        if (document.querySelector('#' + storedAttribute).value != localStorage.getItem(storedAttribute)) {   
            status = true;
        }
    })

    sessionValues.forEach(function(storedAttribute) {
        if (document.querySelector('#' + storedAttribute).value != sessionStorage.getItem(storedAttribute)) {
            status = true;
        }    
    })

    if (countryBox.selectedIndex != parseInt(sessionStorage.getItem("country"))) {
        status = true;
    }

    if (document.querySelector('#weaponGrid input[value=\"' + localStorage.getItem("pirateWeapon") + '\"]').checked != localStorage.getItem("pirateWeapon")) {
        status = true;
    }

    if (document.querySelector('#aboutme').value != null) {
        status = true;
    }
